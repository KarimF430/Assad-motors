...
export default function HorizontalCarCard({ car, showFuelType = false, customLabel }: HorizontalCarCardProps) {
    const exShowroomPrice = car.startingPrice
    const displayPrice = (exShowroomPrice / 100000).toFixed(2)
    const priceLabel = 'Price'

    // Generate slug safely
    const brandSlug = car.brandName?.toLowerCase().replace(/\s+/g, '-') || 'brand'
    const modelSlug = car.name?.toLowerCase().replace(/\s+/g, '-') || 'model'
    const linkHref = `/${brandSlug}-cars/${modelSlug}`
        ...
        >
            <div className="bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-md transition-all duration-200">
                <div className="flex min-h-[144px] sm:min-h-[176px]">
                    {/* Car Image - Responsive - Matches BrandCarCard */}
                            ...
                            onError={(e) => {
                                ...
                                if (parent) {
                                    if (!parent.querySelector('.fallback-text')) {
                                        ...
                                        fallback.className = 'bg-gray-200 text-gray-600 text-xs sm:text-sm font-semibold text-center flex items-center justify-center h-full p-1 fallback-text'
                    ...
                    {/* Car Details - Responsive - Matches BrandCarCard */}
                    <div className="flex-1 p-2 sm:p-3 md:p-4 flex flex-col justify-between min-w-0">
                        ...
                        <div>
                            ...
                            {/* Sub-info: Rating or Custom Label */}
                            <div className="mb-0.5 sm:mb-1">
                                {customLabel ? (
                                    <div className="flex items-center gap-1 sm:gap-1.5">
                                        <Fuel className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-emerald-600 flex-shrink-0" />
                                        <span className="text-emerald-600 font-bold text-xs sm:text-sm whitespace-nowrap">{customLabel}</span>
                                    </div>
                                ) : (
                                    (car.rating || 0) > 0 ? (
                                        <div className="flex items-center gap-0.5 sm:gap-1">
                                            ...
                                            <span className="text-gray-500 text-xs sm:text-sm whitespace-nowrap">{car.reviews || 0} Ratings</span>
                                        </div>
                                    ) : null
                                )}
                            </div>
